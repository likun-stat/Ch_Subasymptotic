...
library(sp)
PNW_west_poly <- data.frame(x = PNW_west$x, y = PNW_west$y)
PNW_west_poly <- rbind(PNW_west_poly, PNW_west_poly[1,])
which.in.polygon.west <- which(point.in.polygon(stationDF_PNW[,1], stationDF_PNW[,2], PNW_west_poly$x, PNW_west_poly$y)==1)
stations_west <- stationDF_PNW[which.in.polygon.west, ]
U <- U[which.in.polygon.west, ]
PNW_west_10Day_maxima <- PNW_JJA_10day[which.in.polygon.west, ]


library(sp)
library(gstat) 
Range <- rep(NA, ncol(PNW_west_10Day_maxima))
for(time in 1:ncol(PNW_west_10Day_maxima)){
  dat_variog <- cbind(stations_west, X= PNW_west_10Day_maxima[,time])
...
delta_m = 0.32
tau_sq_m = 37
corr_m = fields::Matern(d=0.3, range  = 0.3886055, nu=1.5)
res_true<-chi_u(N=1e8,delta=delta_m, tau=tau_sq_m, corr=corr_m) 
...
range_m = 1.15
nu_m=1.68
res_true_IM<-chi_u_invertedMaxStab(N = 1e8, d=0.3, range=range_m, nu=nu_m)
...
plt <- ggplot(dat,aes(x=x,y=truth)) +
  ...
  scale_x_continuous(expand = c(0, 0), limits=c(0.947,1)) + ggtitle("PNW_west") +
  scale_y_continuous(expand = c(0, 0), limits = c(0,1)) + 
  force_panelsizes(rows = unit(3.05, "in"),
                   cols = unit(3.05, "in"))

plt
ggsave("./chi_fit_PNWwest.pdf", width=3.7, height=3.7, unit = 'in')
...
plt
ggsave("./chi_bar_fit_PNWwest.pdf", width=5.3, height=3.7, unit = 'in')


## -------------------------------------------------------
##                        h=0.3
## -------------------------------------------------------
Dist <- fields::rdist(stations_west[,1:2])
h <- 0.3; tol <- 0.05
...
points(stations_west[,1:2], pch=20)
for(iter in 1:nrow(pairs)){
  points(stations_west[pairs[iter,],1:2], type='l')
}
...
dat <- data.frame(x=u_vec, truth=EmpIntv_sim[,3], truth_upper=EmpIntv_sim[,2],truth_lower=EmpIntv_sim[,1], region = "PNW_west")
Dat <- dat
...
dat_bar <- data.frame(x=u_vec, truth=EmpIntv_sim_bar[,3], truth_upper=EmpIntv_sim_bar[,2],truth_lower=EmpIntv_sim_bar[,1], region = "PNW_west")
Dat_bar <- dat_bar
