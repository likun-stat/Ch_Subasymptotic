...
#############################################################################################################################
#############################################################################################################################
load("./PNW_south.RData")
load("./PNW_10Day_copula.RData")
load("./PNW_10Day_maxima.RData")
stationDF_PNW <- stationDF_PNW[,-1]
library(sp)
PNW_south_poly <- data.frame(x = PNW_south$x, y = PNW_south$y)
PNW_south_poly <- rbind(PNW_south_poly, PNW_south_poly[1,])
which.in.polygon.south <- which(point.in.polygon(stationDF_PNW[,1], stationDF_PNW[,2], PNW_south_poly$x, PNW_south_poly$y)==1)
stations_south <- stationDF_PNW[which.in.polygon.south, 1:2]
U <- U[which.in.polygon.south, ]
PNW_south_10Day_maxima <- PNW_JJA_10day[which.in.polygon.south, ]


library(sp)
library(gstat) 
Range <- rep(NA, ncol(PNW_south_10Day_maxima))
for(time in 1:ncol(PNW_south_10Day_maxima)){
  dat_variog <- cbind(stations_south, X= PNW_south_10Day_maxima[,time])
...
delta_m = 0.296
tau_sq_m = 10
corr_m = fields::Matern(d=0.3, range  = 0.292, nu=1.5)
...
plt <- ggplot(dat,aes(x=x,y=truth)) +
  ...
  scale_x_continuous(expand = c(0, 0), limits=c(0.947,1)) + ggtitle("PNW_south") +
  scale_y_continuous(expand = c(0, 0), limits = c(0,1)) + 
  force_panelsizes(rows = unit(3.05, "in"),
                   cols = unit(3.05, "in"))

plt
ggsave("./chi_fit_PNWsouth.pdf", width=3.7, height=3.7, unit = 'in')
...
plt
ggsave("./chi_bar_fit_PNWsouth.pdf", width=5.3, height=3.7, unit = 'in')


## -------------------------------------------------------
##                        h=0.3
## -------------------------------------------------------
Dist <- fields::rdist(stations_south[,1:2])
h <- 0.3; tol <- 0.05
...
points(stations_south[,1:2], pch=20)
for(iter in 1:nrow(pairs)){
  points(stations_south[pairs[iter,],1:2], type='l')
}
...
dat <- data.frame(x=u_vec, truth=EmpIntv_sim[,3], truth_upper=EmpIntv_sim[,2],truth_lower=EmpIntv_sim[,1], region = "PNW_south")
Dat <- dat
...
dat_bar <- data.frame(x=u_vec, truth=EmpIntv_sim_bar[,3], truth_upper=EmpIntv_sim_bar[,2],truth_lower=EmpIntv_sim_bar[,1], region = "PNW_south")
Dat_bar <- dat_bar
