...
GSL-backed numerical integration + root finding for RW mixture CDF/PDF/quantiles.

This file is designed to be compiled into a shared library (`RW_inte_cpp.so`)
and called from Python via ctypes (see RW_inte.py).

---------------------------------------------------------------------------
1) Shifted RW (numerical integration)
---------------------------------------------------------------------------
The functions pRW_transformed() and dRW_transformed() evaluate (respectively)
the CDF and PDF for a RW distribution whose survival/CDF admits an integral
representation over r ∈ (0, ∞).

The code uses the standard compactifying substitution

    r = (1 - t) / t ,   t ∈ (0, 1),     dr = dt / t^2,

so that the improper integral is mapped onto a finite interval (0,1). This
typically improves numerical stability vs integrating directly on (0, ∞).

Quantiles are computed by solving F(x) - p = 0 via Brent's method
(qRW_transformed_brent).

---------------------------------------------------------------------------
2) Standard (non-shifted) Pareto link (incomplete gamma)
---------------------------------------------------------------------------
For the "standard Pareto" link version, the same CDF/PDF can be written in
terms of incomplete gamma functions, which is substantially faster than
numerical integration. These are implemented as:

    pRW_standard_Pareto_C()
    dRW_standard_Pareto_C()

Quantiles again use Brent root finding (qRW_standard_Pareto_C_brent).

---------------------------------------------------------------------------
3) Gaussian nugget (convolution)
---------------------------------------------------------------------------
When a Gaussian nugget ε ~ N(0, tau^2) is added, closed forms are lost and we
evaluate the CDF/PDF via numerical convolution integrals:

    pRW_standard_Pareto_nugget_C()
    dRW_standard_Pareto_nugget_C()

Integration bounds are truncated to [x - 38*tau, x + 38*tau] (with a floor at 0)
because the Gaussian kernel outside ±38 SD is negligible in double precision.
...
Notes:
* All exported functions are wrapped in `extern "C"` for a stable ABI for ctypes.
* We disable GSL's default error handler in several places so we can report
  issues without aborting the process.
*/
...
// `params_ptr` is passed as void* by GSL; we reinterpret it as a pointer
// to a fixed-size array so we can index parameters safely.
...
// ---------------------------------------------------------------------------
// Test driver (NOT used when building the shared library)
// ---------------------------------------------------------------------------
int main(){
