...
The utility functions used by the MCMC sampler and RW-mixture / transformed-data model.

Main dependencies
-----------------
- NumPy / SciPy
- RW_inte.py (and optional C++-accelerated extension built from RW_inte_cpp.cpp)

Notes
-----
This file contains:
- basis/weight functions (Gaussian kernel, Wendland compact support),
- distribution utilities (Lévy, GEV, half-t),
- transformations between Normal and (standard/shifted) Pareto scales,
- one-time marginal likelihood helpers for the transformed RW-mixture model,
- (non)stationary spatial covariance builders,
- a single-time imputation routine for missing values.
...
def weights_fun(d,radius,h=1, cutoff=True):
    ...
    d : float or array-like
        Distances from a target location.
    radius : float
        Hard cutoff radius; weights are set to 0 when d > radius if `cutoff=True`.
    h : float, default=1
        Bandwidth (note: this implementation uses exp(-d^2/(2*h)); i.e., `h` plays the role of
        a variance-like scale rather than a standard deviation).
    cutoff : bool, default=True
        Whether to apply the hard cutoff at `radius`.

    Returns
    -------
    np.ndarray
        Nonnegative weights that sum to 1 (over the input `d`).
    """
...
def wendland_weights_fun(d, theta, k=0, dimension=2, derivative=0):
    """Compute normalized Wendland compactly-supported weights.

    This implements several closed-form Wendland basis functions for dimensions 1–3+,
    with smoothness controlled by `k`. The function is compactly supported on [0, theta]
    (i.e., returns 0 for d >= theta), and then normalized to sum to 1 over the provided `d`.

    Notes
    -----
    - If all distances are >= theta, the unnormalized weights are all zero and the
      normalization will divide by zero. Upstream code should avoid this situation.
    """
...
def rlevy(n, m = 0, s = 1):
    """Sample from a Lévy distribution.

    Uses the inverse-CDF representation:
        X = m + s / Z^2,  Z ~ N(0, 1) with a suitable tail event.
    ...
    m : float, default=0
        Location parameter.
    s : float, default=1
        Scale parameter (must be nonnegative).
...
def dgev(yvals, Loc, Scale, Shape, log=False):
    """GEV density (or log-density) with the scipy `genextreme` sign convention.

    SciPy uses a shape parameter `c` with the opposite sign of the common EVT parameter ξ.
    Here, `Shape` is ξ, and we call SciPy with `c = -Shape`.
    """
    if log:
...
def pgev(yvals, Loc, Scale, Shape, log=False):
    """GEV CDF (or log-CDF) with the scipy `genextreme` sign convention (c = -Shape).
    """
    if log:
...
def qgev(p, Loc, Scale, Shape):
    """GEV quantile function with the scipy `genextreme` sign convention (c = -Shape).
    """
...
def dhalft(y, nu, mu=0, sigma=1):
    """Half-Student-t density (support y >= mu).

    Returns 2 * t_pdf(y; df=nu, loc=mu, scale=sigma) for y >= mu, else 0.
    """
    if y >= mu:
...
def phalft(y, nu, mu=0, sigma=1):
    """Half-Student-t CDF (support y >= mu).

    Returns 2 * t_cdf(y; ...) - 1 for y >= mu, else 0.
    """
    if y >= mu:
...
def rhalft(y, nu, mu=0, sigma=1):
    """Sample from a half-Student-t distribution (absolute value of centered t).
    """
...
def norm_to_stdPareto(Z):
    """Map N(0,1) -> standard Pareto(b=1) via probability integral transform.
    """
...
def stdPareto_to_Norm(W):
    """Map standard Pareto(b=1) -> N(0,1) via probability integral transform.
    """
...
def norm_to_Pareto1(z):
    """Map N(0,1) -> shifted Pareto (support (0,∞)).

    Uses W = 1/(1-Φ(z)) - 1, equivalent to standard Pareto minus 1.
    Clips Φ(z) away from 1 for numerical stability.
    """
...
def pareto1_to_Norm(W):
    """Inverse map for `norm_to_Pareto1` (shifted Pareto -> N(0,1)).
    """
...
def marg_transform_data_mixture_likelihood_1t_shifted(Y, X, Loc, Scale, Shape, phi_vec, gamma_vec, R_vec, cholesky_U):
    """Log marginal likelihood at one time point (shifted Pareto version).

    This evaluates the Gaussian copula / latent-Gaussian term for Z, the Jacobian for the
    transformation X -> W -> Z, and the GEV marginal term for Y given X through the RW
    mixture model (via dRW/pRW/qRW from `RW_inte`).

    Returns
    -------
    float
        Log-likelihood contribution; returns -inf if numerical linear algebra fails.
    """
...
def marg_transform_data_mixture_likelihood_1t_standard(Y, X, Loc, Scale, Shape, phi_vec, gamma_vec, R_vec, cholesky_U):
    """Log marginal likelihood at one time point (standard Pareto version).

    Same structure as the shifted version, but assumes W >= 1 (standard Pareto support).
    Returns -inf if any W < 1 or if numerical linear algebra fails.
    """
    if(isinstance(Y, (int, np.int64, float))): Y=np.array([Y], dtype='float64')
...
def marg_transform_data_mixture_likelihood_1t_detail(Y, X, Loc, Scale, Shape, phi_vec, gamma_vec, R_vec, cholesky_U):
    """Return the one-time log-likelihood broken into additive parts.

    Useful for diagnostics/plotting. The returned vector contains:
        [Gaussian latent term, +Jacobian(Z), +Jacobian(W,X,R), +GEV(Y), -log dRW(X)].
    """
    if(isinstance(Y, (int, np.int64, float))): Y=np.array([Y], dtype='float64')
...
def cov_spatial(r, cov_model = "exponential", cov_pars = np.array([1,1]), kappa = 0.5):
    """Stationary correlation function (exponential / gaussian / matern).

    Parameters
    ----------
    r : array-like
        Nonnegative pairwise distances.
    cov_model : {"exponential","gaussian","matern"}
    cov_pars : np.ndarray
        Placeholder for consistency with original R code; not used here beyond validation.
    kappa : float
        Matern smoothness ν when cov_model="matern".

    Returns
    -------
    np.ndarray
        Correlations with same shape as `r`.
    """
    ...
    if cov_model == "gaussian" :
        C = np.exp(-(r**2))  # NOTE: python uses ** for powers
...
def ns_cov(range_vec, sigsq_vec, coords, kappa = 0.5, cov_model = "matern"):
    """Locally isotropic nonstationary covariance (Paciorek–Schervish style).

    Implements a nonstationary covariance using location-specific range and marginal scale
    parameters, combining:
      - a spatially varying 'scale matrix' term, and
      - an underlying stationary correlation (matern/gaussian/exponential) evaluated at a
        location-dependent Mahalanobis-like distance.

    Returns an N x N covariance matrix.
    """
    ## Arguments:
...
def ns_cov_interp(range_vec, sigsq_vec, coords, tck):
    """Nonstationary covariance with interpolated Matern correlation.

    Same as `ns_cov`, but replaces expensive Bessel-k evaluations with an interpolant `tck`
    (e.g., from `scipy.interpolate.PchipInterpolator`) evaluated at distances.
    Assumes the underlying model is Matern.
    """
...
def impute_1t(miss_index, obs_index, Y_vec, X_vec, mu_vec, sigma_vec, ksi_vec, phi_vec, gamma_vec, R_vec, K):
    ...
    """Impute missing X and Y at one time point using conditional Gaussianity in Z.

    Workflow
    --------
    1) Transform observed X -> W -> Z at observed sites.
    2) Compute conditional distribution of Z at missing sites under K.
    3) Sample Z_miss, transform back to X_miss, then map to Y_miss via RW CDF + GEV quantile.

    Returns
    -------
    (X_miss, Y_miss) : tuple[np.ndarray | None, np.ndarray | None]
        If `miss_index` is empty, returns (None, None).
    """
    if len(miss_index) == 0:
        return (None, None)
...
def impute_1t_fake(miss_index, obs_index, Y_vec, X_vec, mu_vec, sigma_vec, ksi_vec, phi_vec, gamma_vec, R_vec, K):
    """Debug helper: return the 'true' missing values (no imputation).
    """
